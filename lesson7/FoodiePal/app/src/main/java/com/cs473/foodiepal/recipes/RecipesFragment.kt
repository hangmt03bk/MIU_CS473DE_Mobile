...
class RecipesFragment : Fragment(), RecipesDialog.OnAddRecipeListener {
    ...
    override fun onCreateView(
        ...
        recipeAdapter =  RecipesAdapter(UserData.user.recipes)
    ...
    override fun onRecipeAdd(recipe: Recipes) {
        UserData.user.recipes.add(recipe)
        recipeAdapter.notifyItemInserted(UserData.user.recipes.size-1)
    ...
    override fun onRecipeRemove() {
        UserData.user.recipes.removeAt(UserData.user.recipes.size-1)
        recipeAdapter.notifyDataSetChanged()
    }
}