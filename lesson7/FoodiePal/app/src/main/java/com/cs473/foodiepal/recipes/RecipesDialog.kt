...
class RecipesDialog: DialogFragment() {
    ...
    private fun onAdd() {
        ...
        builder.setIcon(R.drawable.recipes)
    ...
    private fun onAddExec() {
        ...
        val imgId = resources.getIdentifier("recipebook", "drawable", activity?.packageName)

        val listener = targetFragment  as? OnAddRecipeListener
        listener?.onRecipeAdd(Recipes(UserData.user.recipes.size+1,name,ingr,cookingTime,rating, imgId))
         dismiss()
    }
